...
This Phase 1 v100 prototype is a compact static web app with no build step. Open `index.html` directly or publish the repository through GitHub Pages.
...
- Nadi Build Tracker that separates product build progress from launch readiness, shows the current sprint, completed modules, launch gates, and Phase 2 distributor roadmap inside the app.
- Nadi Launch Readiness Board that turns launch readiness into production blockers across live data, source QA, accounts, payments, compliance, security, support, and Phase 2 boundaries.
...
- Nadi Payment Wiring Console that maps hosted checkout, monthly and annual plans, webhook verification, entitlement activation, invoice posture, refunds, cancellation, duplicate webhooks, and settlement reconciliation before paid launch.
- Nadi Payment Gateway Sandbox Route that dry-runs hosted checkout, signed webhooks, duplicate events, failed renewals, refunds, invoices, entitlement activation, support repair, incidents, and reconciliation before any real payment rail is switched on.
...
- Nadi Subscription Ops Console that maps renewals, failed payments, refunds, invoices, support repair, reconciliation, incidents, and redacted receipts before paid retail launch.
- Nadi Subscription Backend Blueprint that maps hosted gateway adapters, signed webhooks, idempotency, event replay, entitlement projection, invoice and refund ledger, support repair, reconciliation, and incident replay before paid launch.
...
- Nadi Retail Account Launch Route that turns auth choice, account storage, saved research migration, paid entitlement join, export/delete, support redaction, monitoring, and the Phase 2 firewall into one founder-readable beta route.
...
- Nadi Source Intake Console that prepares official URLs, files, API feeds, field contracts, source dates, citation paths, validation checks, and privacy boundaries before a source enters live dry run.
...
- Nadi Source Receipt Vault that saves browser-local dry-run source receipts with source metadata, proof scores, blockers, freeze rules, reviewer handoff, and privacy boundaries.
...
- Nadi Backend Audit Receipts that turns reviewer release, source ingestion, saved research, payment entitlement, privacy consent, and support events into backend receipt contracts with idempotency, redaction, retention, route, and launch-blocker checks.
