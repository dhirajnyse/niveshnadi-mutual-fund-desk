...
This Phase 1 v130 prototype is a compact static web app with no build step. Open `index.html` directly or publish the repository through GitHub Pages.
...
- Nadi Entitlement Bridge that connects Free Starter, Nadi Plus Monthly, Nadi Plus Annual, Founder Beta, grace, refund, expired, and support states to research feature access before paid accounts launch.
...
- Nadi Subscription Backend Blueprint with Backend Support Receipts that maps hosted gateway adapters, signed webhooks, idempotency, event replay, entitlement projection, invoice and refund ledger, support repair, support receipt families, redacted lookup, repair action replay, reconciliation, and incident replay before paid launch.
...
- Nadi Account Vault Blueprint with a Backend Storage Handoff Board that converts browser-local research into backend-owned row contracts with migration batch, handoff receipt, restore proof, export/delete proof, deletion receipt, support redaction, and rollback boundaries.
...
- Nadi Account Vault Blueprint that defines saved research collections, entitlement guards, audit events, export/delete workflow, support redaction, and Phase 2 data exclusions before paid accounts launch.
...
- Nadi Backend Audit Receipts with Production Source Import Jobs, Source Import Worker Blueprint, Monitoring Alert Routing, Alert Delivery Backend, Failed-Run Event Store, Reviewer Sign-off Bridge, Rollback Evidence Store, Public Recovery Rehearsal, Recovery Release Queue, Incident Receipt Replay, and Payment Reconciliation Replay that turns reviewer release, source ingestion, saved research, payment entitlement, privacy consent, support, invoice, refund, settlement, and entitlement events into backend receipt contracts with idempotency, redaction, retention, source job IDs, import gate IDs, parser/schema versions, worker IDs, retry policy, monitoring alerts, owner routing, notification channels, delivery attempts, dead-letter queues, reviewer escalation, failed-run envelopes, rejected-row digests, replay cursors, reviewer posture, release scopes, rollback binders, correction receipts, resume receipts, surface recovery proof, public recovery plan IDs, correction publish IDs, freeze/resume IDs, recovery queue IDs, publish batch IDs, correction/support/monitor task IDs, support-safe summaries, release queue rehearsal, event sequences, acknowledgement windows, incident receipt fields, failed-run replay, freeze actions, reviewer closeout, pause rules, rejected-row quarantine, rollback policy, replay scripts, variance checks, and launch-blocker checks.
