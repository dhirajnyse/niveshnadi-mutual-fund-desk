...
This Phase 1 v292 prototype is a compact static web app with no build step. Open `index.html` directly or publish the repository through GitHub Pages.
...
- Nadi Backend Audit Receipts with Backend Source Receipt Job, Scheduled Worker Receipt Contract, Production Source Import Jobs, Source Import Worker Blueprint, Monitoring Alert Routing, Alert Delivery Backend, Failed-Run Event Store, Reviewer Sign-off Bridge, Rollback Evidence Store, Public Recovery Rehearsal, Recovery Release Queue, Correction Publish Console, Incident Receipt Replay, Payment Reconciliation Replay, and Launch Freeze Automation that turns reviewer release, source ingestion, saved research, payment entitlement, privacy consent, support, invoice, refund, settlement, entitlement, and freeze/resume events into backend receipt contracts with idempotency, redaction, retention, source receipt job IDs, worker contract IDs, worker ticket IDs, acceptance pack IDs, source job IDs, import gate IDs, parser/schema versions, worker IDs, retry policy, monitoring alerts, owner routing, notification channels, delivery attempts, dead-letter queues, reviewer escalation, failed-run envelopes, rejected-row digests, replay cursors, reviewer posture, release scopes, rollback binders, correction receipts, resume receipts, surface recovery proof, public recovery plan IDs, correction publish IDs, correction console IDs, publication attempt IDs, investor-visible receipt IDs, wording compare, publish/hold/rollback posture, monitoring windows, freeze/resume IDs, recovery queue IDs, publish batch IDs, correction/support/monitor task IDs, support-safe summaries, launch-freeze IDs, trigger gates, resume rules, release queue rehearsal, event sequences, acknowledgement windows, incident receipt fields, failed-run replay, freeze actions, reviewer closeout, pause rules, rejected-row quarantine, rollback policy, replay scripts, variance checks, and launch-blocker checks.
