...
The Nadi Privacy Control Room is the Phase 1 transparency layer for browser-local data. As the product grows into saved reviews, dossiers, receipts, memory capsules, watchlists, alerts, briefing snapshots, and research memory, the user needs one clear place to see what the prototype has stored on the current device.
