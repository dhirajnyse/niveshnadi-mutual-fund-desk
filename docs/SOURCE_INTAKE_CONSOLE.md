...
V296 uses those intake fields inside Backend CI Smoke Harness. Scheduler, parser, receipt persistence, alert delivery, failed-run replay, and public recovery closeout fixtures now assert source job ID, worker ID, parser/schema versions, accepted and rejected counts, source receipt IDs, replay cursor, and blocked-data attestation before the backend gate can pass.

V297 carries those same source and smoke proofs into Founder Beta Recovery Rehearsal. Source job ID, Trust Center update ID, investor-visible receipt, support-safe script, monitor ID, and no-private-data attestation must agree before founder beta users see a recovery state.
...
V300 carries source import proof into Backend Deploy Runbook Packet. Source job IDs, parser/schema versions, accepted and rejected counts, failed-run replay, receipt persistence, alert delivery, and release evidence now become deploy command proof and environment checks before a source worker can move toward production.
