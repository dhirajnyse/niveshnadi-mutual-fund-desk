...
V115 adds Backend Support Receipts. The blueprint now binds support tooling into receipt families for case open, redacted lookup, repair request, backend action, finance link, replay script, closeout reason, allowed fields, and blocked private identifiers.

V150 adds Payment Adapter Repairs. The blueprint now maps checkout, signed webhook, idempotency replay, refund and cancellation, entitlement projection, support repair, settlement, and incident adapters into one reversible route. Each adapter names event in, event out, receipt join, repair action, rollback rule, owner, and launch blocker before paid access can rely on a provider event.
