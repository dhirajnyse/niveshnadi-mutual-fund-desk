...
The visible header now prioritizes the primary workflow: Screener, Playbook, Compare, X-Ray, Review, Dossier, and Pricing. All other tools remain available through the Workspace dropdown, grouped by Discover, Compare and Fit, Portfolio, and Trust and Follow-up. The command status chip shows the active research zone on wider screens, while primary shortcuts receive an active state as the user moves through the workspace.
